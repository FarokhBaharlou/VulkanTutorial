...
{
	template<>
	struct hash<MyEngine::Model::Vertex>
	{
		size_t operator()(MyEngine::Model::Vertex const& vertex) const
		{
			size_t seed = 0;
			MyEngine::hashCombine(seed, vertex.position, vertex.color, vertex.normal, vertex.uv);
			return seed;
		}
	};
}
